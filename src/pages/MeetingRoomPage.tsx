...
import { Video, VideoOff, Calendar, Users, Mic, MicOff, Monitor, Share2, MessageSquare, Phone, Clock, CheckCircle, Videotape, X, Send, MoreVertical, Hand, Smile, FileText, ChevronUp, Settings, Layout, Maximize2, Minimize2, Bot, Sparkles, Loader2 } from 'lucide-react';
import { useState, useEffect, useRef, useCallback } from 'react';
import { useTheme } from '../lib/ThemeContext';
import { useTranscription } from '../hooks/meeting/useTranscription';
import { useAISummarizer } from '../hooks/meeting/useAISummarizer';
import { useVirtualParticipant } from '../hooks/meeting/useVirtualParticipant';
...
const features = [
  ...
  { icon: Bot, title: 'AI Assistant', description: 'Get AI-powered notes, summaries, and assistance during meetings' },
  { icon: Sparkles, title: 'AI Transcription', description: 'Live speech-to-text transcription with AI summaries' },
];
...
export function MeetingRoomPage() {
  ...
  const handleGenerateSummary = async () => {
    await generateSummary(transcript, 'Team Meeting');
  };
...
ACTION ITEMS
${summary.actionItems.map(a => `- ${a.description} ${a.assignee ? `(Assigned: ${a.assignee})` : ''}`).join('\n')}
...
${summary.nextSteps.map(s => `- ${s}`).join('\n')}
  ...
  const handleExportDOCX = () => {
    // Same as PDF for now - export as text
    handleExportPDF();
  };
          ...
          <div className="bg-dark-900 border border-dark-700 rounded-3xl p-8 w-full max-w-lg mx-4 shadow-2xl">
            ...
            <div className="mt-6 p-4 rounded-xl bg-dark-800/50 border border-dark-800">
              <div className="flex items-center gap-3 text-sm text-dark-400">
                ...
                <span>HD video and audio • AI Transcription • Smart summaries</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div ref={meetingRef} className="fixed inset-0 z-50 flex flex-col bg-dark-950">
          ...
          <div className="h-16 bg-gradient-to-r from-dark-900 via-dark-900/95 to-dark-900 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* AI Indicators */}
              {transcript.length > 0 && (
                <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-primary-500/20 rounded-xl border border-primary-500/30">
                  <Bot className="w-4 h-4 text-primary-400" />
                  <span className="text-primary-400 text-xs font-medium">{transcript.length} segments</span>
                </div>
              )}
              ...
              <button onClick={() => setShowSummaryModal(true)} className="px-4 py-2 bg-gradient-to-r from-primary-500 to-accent-500 text-white rounded-xl hover:from-primary-600 hover:to-accent-600 transition-all text-sm font-medium flex items-center gap-2">
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 p-4 lg:p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 h-full">
                ...
                {/* Participant Views */}
                {(meetingMode === 'demo' ? demoParticipants : demoParticipants.slice(0, 2)).map((participant) => (
                  <div
                    key={participant.id}
                ...
                {/* AI Assistant Tile */}
                {isAIEnabled && (
                  <div className="relative bg-gradient-to-br from-primary-900/30 to-accent-900/30 rounded-3xl overflow-hidden border border-primary-500/30 shadow-2xl shadow-primary-500/10">
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <div className="w-20 h-20 bg-gradient-to-br from-primary-500 via-accent-500 to-primary-500 rounded-full flex items-center justify-center shadow-2xl shadow-primary-500/30 mb-4">
                        <Bot className="w-10 h-10 text-white" />
                      </div>
                      <span className="text-white font-semibold">AI Assistant</span>
                      <span className="text-dark-400 text-xs mt-1">Online</span>
                    </div>
                    <div className="absolute bottom-4 left-4">
                      <div className="px-3 py-1.5 bg-dark-900/80 backdrop-blur-md rounded-xl border border-primary-500/30">
                        <span className="text-primary-400 text-xs font-medium">Helping</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {/* AI Panel - Transcription & Assistant */}
            {showAIPanel && (
              <div className="w-96 bg-gradient-to-b from-dark-900 to-dark-950 border-l border-white/5 flex flex-col">
                <div className="p-4 border-b border-white/5">
                  <div className="flex gap-2 p-1.5 bg-dark-800/50 rounded-xl">
                    <button
                      onClick={() => { setShowTranscription(true); setShowAIAssistant(false); }}
                      className={`flex-1 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${showTranscription && !showAIAssistant ? 'bg-gradient-to-r from-primary-500 to-accent-500 text-white shadow-lg' : 'text-dark-400 hover:text-white hover:bg-dark-700'}`}
                    >
                      Transcript
                    </button>
                    <button
                      onClick={() => { setShowTranscription(false); setShowAIAssistant(true); }}
                      className={`flex-1 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${showAIAssistant ? 'bg-gradient-to-r from-primary-500 to-accent-500 text-white shadow-lg' : 'text-dark-400 hover:text-white hover:bg-dark-700'}`}
                    >
                      AI Assistant
                    </button>
                  </div>
                ...
                {showTranscription && !showAIAssistant ? (
                  <div className="flex-1 overflow-hidden">
                    <TranscriptionPanel
                      transcript={transcript}
                      isListening={isListening}
                      interimText={interimText}
                      onToggleListening={toggleTranscription}
          ...
          <div className="h-24 bg-gradient-to-t from-dark-900 via-dark-900/95 to-dark-900/90 backdrop-blur-xl border-t border-white/5 flex items-center justify-center px-4">
            <div className="flex items-center gap-1 md:gap-2 bg-dark-800/80 backdrop-blur-xl rounded-2xl px-3 py-2 border border-white/10 shadow-2xl">
              ...
              {/* AI Panel Toggle */}
              <button
                onClick={() => { setShowAIPanel(!showAIPanel); setShowTranscription(true); setShowAIAssistant(false); }}
                className={`p-3 md:p-4 rounded-xl transition-all duration-300 ${showAIPanel ? 'bg-gradient-to-br from-primary-500 to-primary-600 text-white shadow-lg shadow-primary-500/25' : 'bg-dark-700/50 text-dark-300 hover:bg-dark-600 hover:text-white'}`}
                title="AI Features"
              >
              ...
              <button
                onClick={() => { setShowAIPanel(true); setShowTranscription(true); setShowAIAssistant(false); }}
                className={`p-3 md:p-4 rounded-xl transition-all duration-300 ${isListening ? 'bg-gradient-to-br from-success-500 to-success-600 text-white shadow-lg shadow-success-500/25' : 'bg-dark-700/50 text-dark-300 hover:bg-dark-600 hover:text-white'}`}
                title={isListening ? 'Stop Transcription' : 'Start Transcription'}
              >
                {isListening ? <Loader2 className="w-5 h-5 md:w-6 md:h-6 animate-spin" /> : <FileText className="w-5 h-5 md:w-6 md:h-6" />}
              </button>
              ...
              {/* Chat Toggle */}
              <button
                onClick={() => { setShowChat(!showChat); setShowParticipants(false); setShowAIPanel(false); }}
                className={`p-3 md:p-4 rounded-xl transition-all duration-300 ${showChat && !showAIPanel ? 'bg-gradient-to-br from-primary-500 to-primary-600 text-white shadow-lg shadow-primary-500/25' : 'bg-dark-700/50 text-dark-300 hover:bg-dark-600 hover:text-white'}`}
                title="Chat"
              >
              ...
              {/* Participants */}
              <button
                onClick={() => { setShowParticipants(!showParticipants); setShowChat(false); setShowAIPanel(false); }}
                className={`p-3 md:p-4 rounded-xl transition-all duration-300 ${showParticipants && !showAIPanel ? 'bg-gradient-to-br from-primary-500 to-primary-600 text-white shadow-lg shadow-primary-500/25' : 'bg-dark-700/50 text-dark-300 hover:bg-dark-600 hover:text-white'}`}
                title="Participants"
              >
